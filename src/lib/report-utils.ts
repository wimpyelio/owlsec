import type { Finding, ScanReport, Severity } from "./scan.functions";
...
export function toMarkdown(report: ScanReport, edits: Record<string, ReviewerEdit>, mode: "security"|"plain", clientName?: string): string {
  ...
  lines.push(`\n## 5. Findings`);
  const active = report.findings.filter(f => !edits[f.id]?.dismissed);
  ...
  const dismissed = report.findings.filter(f => edits[f.id]?.dismissed);
  if (dismissed.length) {
...
export function _typecheck(_f: Finding) { return _f; }
