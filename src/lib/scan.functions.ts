...
// ─── Structured logger (emits JSON lines; swap for your APM with no other changes) ───
const log = {
...
// Works in a single-process deployment (Cloudflare Workers / Node SSR).
// Replace with Redis/Upstash if you go multi-instance.
const RATE_WINDOW_MS  = 60_000; // 1 min window
const RATE_MAX_REQS   = 5;      // max scans per IP per window
...
function checkRateLimit(ip: string): void {
  ...
  // Prune old IPs to avoid unbounded memory growth
...
// ─── Gap #4 — Prompt injection defence ────────────────────────────────────────
// User-supplied content is wrapped in hard delimiters with an explicit instruction
// telling the model to treat everything inside as DATA, never as instructions.
// This won't stop a determined adversary, but it raises the bar significantly and
// is the standard mitigation for indirect prompt injection in static-analysis tools.
...
function buildUserPayload(input: ScanInput): string {
  const parts: string[] = [];
  // system_prompt is the subject of the scan — still wrap it as untrusted data
...
async function callModel(
  ...
  // ── Gap #1 (ship-blocker) — safe JSON parse with one automatic retry ────────
  let parsed: unknown;
  try {
...
export const runScan = createServerFn({ method: "POST" })
  .inputValidator((data: ScanInput) => {
    ...
    if (data.system_prompt.length > 20000)
      throw new Error("system_prompt exceeds 20,000 characters");
    if (data.tool_config && data.tool_config.length > 100_000)
      throw new Error("tool_config exceeds 100KB");
    if (data.architecture && data.architecture.length > 2000)
      throw new Error("architecture exceeds 2,000 characters");
    if (data.code_files && data.code_files.length > 5)
      throw new Error("max 5 code files");
    return data;
  })
  .handler(async ({ data, context }): Promise<ScanReport> => {
    const requestId = crypto.randomUUID();
    const scanStart = Date.now();

    // ── Gap #1 (ship-blocker) — rate limiting ───────────────────────────────
    // TanStack Start exposes the raw Request on context.request (Cloudflare/Node)
    const ip =
      (context as unknown as { request?: Request })?.request?.headers?.get("cf-connecting-ip") ??
      (context as unknown as { request?: Request })?.request?.headers?.get("x-forwarded-for")?.split(",")[0].trim() ??
      "unknown";
    checkRateLimit(ip);
